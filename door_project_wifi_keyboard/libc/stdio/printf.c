...
typedef void (*out_fct_type)(char character, void* buffer, size_t idx, size_t maxlen, uint32_t uart_num);
...
static inline void _out_null(char character, void* buffer, size_t idx, size_t maxlen, uint32_t uart_num)
{
    (void)character; (void)buffer; (void)idx; (void)maxlen;(void)uart_num;
}

// internal _putchar wrapper
//static inline void _out_char(char character, void* buffer, size_t idx, size_t maxlen)// initial
static inline void _out_char(char character, void* buffer, size_t idx, size_t maxlen, uint32_t uart_num)
{
    (void)buffer; (void)idx; (void)maxlen;
    if (character)
    {
        _putchar(character, uart_num);
    }
}
...
static size_t _out_rev(out_fct_type out, char* buffer, size_t idx, size_t maxlen,
	const char* buf, size_t len, unsigned int width, unsigned int flags, uint32_t uart_num)
{
    size_t i;
    ...
    if (!(flags & FLAGS_LEFT) && !(flags & FLAGS_ZEROPAD))
    {
        for (i = len; i < width; i++)
        {
            out(' ', buffer, idx++, maxlen, uart_num);
        }
    }
    ...
    while (len)
    {
        out(buf[--len], buffer, idx++, maxlen, uart_num);
    }
...
static size_t _ntoa_format(out_fct_type out, char* buffer, size_t idx, size_t maxlen,
	char* buf, size_t len, bool negative, unsigned int base,
		unsigned int prec, unsigned int width, unsigned int flags, uint32_t uart_num)
{
    // pad leading zeros
    if (!(flags & FLAGS_LEFT))
    {
    ...
    return _out_rev(out, buffer, idx, maxlen, buf, len, width, flags, uart_num);
}
		...
		unsigned int prec, unsigned int width, unsigned int flags, uint32_t uart_num)
{
    ...
    return _ntoa_format(out, buffer, idx, maxlen, buf, len, negative, (unsigned int)base, prec, width, flags, uart_num);
}
...
// internal vsnprintf
// in printk(), use like this : _vsnprintf(_out_char, buffer, (size_t)-1, format, va);
static int _vsnprintf(out_fct_type out, char* buffer, const size_t maxlen, const char* format, va_list va, uint32_t uart_num)
{
    ...
    while (*format)
    {
        ...
        {
            // no
            out(*format, buffer, idx++, maxlen, uart_num);// out 是 _out_char() 函数，直接输出 *format 当前的一个字符。
            format++;// 下一个字符。
            continue;// 重新开始循环。也就是如果要打印的字符没有'%'，那么直接输出就好了，不会执行后面的代码。
        }
        else
        {
            // yes, evaluate it
        ...
        switch (*format)
        {
            ...
            case 'b' :
            {
                ...
                if ((*format == 'i') || (*format == 'd'))
                {
                    ...
                    else if (flags & FLAGS_LONG)
                    {
                        ...
                        idx = _ntoa_long(out, buffer, idx, maxlen, (unsigned long)(value > 0 ? value : 0 - value),
                                         value < 0, base, precision, width, flags, uart_num);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        idx = _ntoa_long(out, buffer, idx, maxlen, va_arg(va, unsigned long), false, base, precision, width, flags, uart_num);
                    }
                    else
                    {
                        ...
                        idx = _ntoa_long(out, buffer, idx, maxlen, value, false, base, precision, width, flags, uart_num);
                    }
                }
            ...
            case 'c' :
            {
                ...
                // char output
                out((char)va_arg(va, int), buffer, idx++, maxlen, uart_num);
                // post padding
                if (flags & FLAGS_LEFT)
                {
                    while (l++ < width)
            ...
            case 's' :
            {
                ...
                {
                    out(*(p++), buffer, idx++, maxlen, uart_num);
                }
            ...
            case 'p' :
            {
                ...
                else
                {
                     ...
                     //idx = _ntoa_long(out, buffer, idx, maxlen, (unsigned long)((uintptr_t)va_arg(va, void*)),
                                      //false, 16U, precision, width, flags);// initial, false
                     idx = _ntoa_long(out, buffer, idx, maxlen, (unsigned long)((uint32_t)va_arg(va, void*)),
                                      false, 16U, precision, width, flags, uart_num);
    #if defined(PRINTF_SUPPORT_LONG_LONG)
                }
    #endif
            ...
            default :
                out(*format, buffer, idx++, maxlen, uart_num);
                format++;
                break;
        }
    }
    ...
    // termination
    out((char)0, buffer, idx < maxlen ? idx : maxlen - 1U, maxlen, uart_num);
...
int printf(const char* format, ...)
{
    ...
    va_start(va, format);
    char buffer[1];
    uint32_t uart_num = 0;// 使用串口1打印
    const int ret = _vsnprintf(_out_char, buffer, (size_t)-1, format, va, uart_num);
    va_end(va);
    return ret;
}
...
//{
    ...
    //const int ret = _vsnprintf(_out_fct, (char*)(uint32_t)&out_fct_wrap, (size_t)-1, format, va);
    //va_end(va);
    //return ret;
//}
...
int printk(const char* format, ...)
{
    ...
    va_start(va, format);
    char buffer[1];
    uint32_t uart_num = 1;// 使用串口1打印
    const int ret = _vsnprintf(_out_char, buffer, (size_t)-1, format, va, uart_num);
    va_end(va);
    return ret;
}

int usart0_print(const char* format, ...)// format 存放引号""内部的东西，如果用到%这样的符号，那么引号""外还需要有与之对应的数。
{
    va_list va;
    va_start(va, format);
    char buffer[1];
    ...
    const int ret = _vsnprintf(_out_char, buffer, (size_t) - 1, format, va, uart_num);
    /*
    把 _out_char() 函数作为参数传入。_out_char() 等价于 'console.c' 中的 console_putch() ，打印字符。
...
int usart2_print(const char* format, ...)// format 存放引号""内部的东西，如果用到%这样的符号，那么引号""外还需要有与之对应的数。
{
    va_list va;
    va_start(va, format);
    char buffer[1];
